<div class="max-w-md w-full mx-auto p-4 sm:p-6  rounded-2xltransition-colors duration-300">
    <h2 class="text-2xl font-extrabold text-green-600 dark:text-green-400 mb-6 text-center">
    ...
    <form action="{{ $psp === 'stripe' ? route('payment.donate.stripe') : route('payment.donate.notchpay') }}" method="POST" class="space-y-5">
        ...
        <div class="grid grid-cols-1 sm:grid-cols-2 gap-4">
            ...
            <div>
                ...
                <input type="text" id="prenom" name="prenom" wire:model.blur="prenom" value="{{ old('prenom') }}"
                       class="mt-1 block w-full border-2 border-gray-300 dark:border-gray-600 dark:bg-gray-700 dark:text-gray-200 rounded-lg shadow-sm focus:border-green-500 focus:ring-green-500 sm:text-sm p-2.5 transition-colors duration-300">
        ...
        @if($psp === 'stripe')
            @if($isConverting)
                <div class="p-3 bg-gray-100 dark:bg-gray-700 rounded-lg text-sm text-gray-600 dark:text-gray-300 flex items-center transition-colors duration-300" role="status">
                    <svg class="animate-spin h-4 w-4 mr-3 text-green-500" xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24">
            ...
            @elseif($convertedAmount && $montant > 0)
                <div class="p-3 bg-green-50 dark:bg-green-900/40 rounded-lg text-sm text-green-700 dark:text-green-300 transition-colors duration-300 border border-green-200 dark:border-green-800">
            ...
            <div class="grid grid-cols-1 gap-3 sm:grid-cols-2">
                @if($stripeEnabled)
                    <label class="flex items-center p-4 border rounded-xl cursor-pointer transition-all duration-300
                                  ...
                                  {{ $psp === 'stripe' ? 'border-green-600 bg-green-50 dark:bg-gray-700' : 'border-gray-300 dark:border-gray-600 dark:bg-gray-800' }}">
                        <input type="radio" wire:model.live="psp" value="stripe" class="form-radio h-4 w-4 text-green-600 focus:ring-green-500 dark:bg-gray-700 dark:border-gray-600">
                        <span class="ml-3 flex-1 flex items-center">
                            <svg class="w-7 h-7 text-green-600 dark:text-green-400 mr-2" fill="none" stroke="currentColor" viewBox="0 0 24 24" xmlns="http://www.w3.org/2000/svg"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M3 10h18M7 15h1m4 0h1m-7 4h12a3 3 0 003-3V8a3 3 0 00-3-3H6a3 3 0 00-3 3v8a3 3 0 003 3z"></path></svg>
                ...
                @if($notchpayEnabled)
                    <label class="flex items-center p-4 border rounded-xl cursor-pointer transition-all duration-300
                                  hover:bg-green-50 dark:hover:bg-gray-700
                                  {{ $psp === 'notchpay' ? 'border-green-600 bg-green-50 dark:bg-gray-700' : 'border-gray-300 dark:border-gray-600 dark:bg-gray-800' }}">
                        <input type="radio" wire:model.live="psp" value="notchpay" class="form-radio h-4 w-4 text-green-600 focus:ring-green-500 dark:bg-gray-700 dark:border-gray-600">
                        <span class="ml-3 flex-1 flex items-center">
        ...
        <button type="submit"
                class="w-full bg-green-600 text-white py-3 mt-6 rounded-lg font-bold text-lg hover:bg-green-700 transition-all duration-300 shadow-lg disabled:opacity-50 dark:disabled:opacity-40"
