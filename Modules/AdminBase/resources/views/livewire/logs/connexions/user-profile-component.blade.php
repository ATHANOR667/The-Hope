    ...
    <div class="bg-white dark:bg-gray-800 rounded-2xl shadow-xl dark:shadow-xl dark:shadow-indigo-900/10 p-8
        ...
        <div class="flex-shrink-0 relative">
            ...
            <span class="absolute bottom-0 right-0 block h-5 w-5 rounded-full ring-4 ring-white dark:ring-gray-800
                          {{ $stats['active_sessions'] > 0 ? 'bg-green-500' : 'bg-gray-400' }}"
    ...
    <div class="bg-white dark:bg-gray-800 rounded-2xl shadow-xl p-6 border border-gray-100 dark:border-gray-700/50">
        <div class="flex flex-col sm:flex-row justify-between items-start sm:items-center mb-6 border-b dark:border-gray-700/50 pb-4">
            ...
            <button wire:click="exportCsv"
                    class="px-4 py-2 bg-green-600 text-white font-medium rounded-lg hover:bg-green-700 transition-colors duration-300 flex items-center text-sm shadow">
                    ...
                    <tr class="hover:bg-gray-50 dark:hover:bg-gray-700/50 transition duration-150">
                        ...
                        <td class="px-6 py-4 whitespace-nowrap">
                            @if ($log->session_end)
                                ...
                                <span class="text-xs text-green-600 dark:text-green-400 font-medium">Durée: {{ $log->session_start->diffForHumans($log->session_end, ['parts' => 2, 'join' => true, 'short' => true]) }}</span>
                            @else
                        ...
                        <div class="flex justify-between items-center">
                            ...
                            <span class="text-right">
                                @if ($log->session_end)
                                    ...
                                    <span class="block text-xs text-green-600 dark:text-green-400">Durée: {{ $log->session_start->diffForHumans($log->session_end, ['parts' => 2, 'join' => true, 'short' => true]) }}</span>
                                @else
