...
use Modules\Shared\Traits\WithPasscodeVerification;
use Livewire\Attributes\On;

class ManageHomePage extends Component
{
    use WithFileUploads;
    use WithPasscodeVerification;
    ...
    #[On('execute-save-founder')]
    public function saveFounder($index, string $token): void
    {
        try {
            $this->getVerifiedSudoToken($token, guard: 'admin');
...
// ou dans un trait qu'elle utilise.
    ...
    #[On('execute-save-meta')]
    public function saveMeta(string $token): void
    {
        try {
            $this->getVerifiedSudoToken($token, guard: 'admin');
        ...
        $this->dispatch('flash-success', 'Informations SEO & OG sauvegardées avec succès');
        $this->dispatch('$refresh');
        } catch (\Exception $e) {
            $this->dispatch('flash-error', $e->getMessage());
        }
    ...
    #[On('execute-save-hero')]
    public function saveHero(string $token): void
    {
        try {
            $this->getVerifiedSudoToken($token, guard: 'admin');
        ...
        if ($embedUrl) {
            ...
            $this->dispatch('flash-success', 'Section Hero (vidéo) mise à jour avec succès');
            $this->dispatch('$refresh');
        } else {
    ...
    #[On('execute-save-social-links')]
    public function saveSocialLinks(string $token): void
    {
        try {
            $this->getVerifiedSudoToken($token, guard: 'admin');
        ...
        $this->saveContent(['social_links' => $assoc]);
        $this->dispatch('flash-success', 'Réseaux sociaux de l\'organisation mis à jour');
        $this->dispatch('$refresh');
        } catch (\Exception $e) {
            $this->dispatch('flash-error', $e->getMessage());
        }
