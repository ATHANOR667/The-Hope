...
<header class="bg-indigo-600 dark:bg-indigo-800 text-white p-4 shadow-md transition-colors duration-300 ease-in-out">
    <div class="container mx-auto flex justify-between items-center">
        ...
        <button @click="darkMode = !darkMode; localStorage.setItem('theme', darkMode ? 'dark' : 'light')"
                class="p-2 rounded-full bg-indigo-700 dark:bg-indigo-900 text-white hover:bg-indigo-800 dark:hover:bg-indigo-700 focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-offset-indigo-600 dark:focus:ring-offset-indigo-800 focus:ring-white transition-colors duration-300 ease-in-out"
...
<main class="flex-grow container mx-auto p-4 flex flex-col items-center justify-center">
    <div class="w-full max-w-sm md:max-w-md mb-6">
        ...
        @if (session('message'))
            <div x-data="{ show: true }" x-show="show" x-transition:leave.opacity.duration.500ms
                 class="bg-indigo-100 dark:bg-indigo-800 border border-indigo-400 dark:border-indigo-700 text-indigo-700 dark:text-indigo-100 px-4 py-3 rounded relative shadow-md flex items-center justify-between" role="alert">
                <span class="block sm:inline">{{ session('message') }}</span>
                <button @click="show = false" class="ml-4 text-indigo-700 dark:text-indigo-100 hover:text-indigo-900 dark:hover:text-indigo-300 focus:outline-none">
                    <svg class="h-5 w-5" role="button" xmlns="http://www.w3.org/2000/svg" viewBox="0 0 20 20" fill="currentColor">
                        <path fill-rule="evenodd" d="M4.293 4.293a1 1 0 011.414 0L10 8.586l4.293-4.293a1 1 0 111.414 1.414L11.414 10l4.293 4.293a1 1 0 01-1.414 1.414L10 11.414l-4.293 4.293a1 1 0 01-1.414-1.414L8.586 10 4.293 5.707a1 1 0 010-1.414z" clip-rule="evenodd" />
                    </svg>
                </button>
